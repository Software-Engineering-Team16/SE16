...
public:
	SearchJob();
	JobAdList* searchJob(const char* name); //회사 이름으로 채용정보 찾기
};
...
public:
	Apply();
	ApplyInfo* createApplyInfo(const char *SSN); //SSN으로 채용 정보 찾기
...
public:
	CancelApplyInfo();
	JobAd* getJobAdDetail(const char* SSN);
	const char* cancelApply(const char *SSN); //취소할 지원 정보의 문자열 반환
};
