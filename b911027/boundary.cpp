...
void ApplyUI::apply()
{
	...
	//cout << applyInfo->getName() << endl;
	//cout << applyInfo->getSSN() << endl;
	//cout << applyInfo->getNumberOfMember() << endl;
	fprintf(out_fp, "> %s %s %s\n", applyInfo->getName(), applyInfo->getSSN(), applyInfo->getTask());
	//회사 이름 사업자번호 업무 출력
	//ApplyInfoList* applyInfoList;
}//ApplyUI
CheckApplyInfoUI::CheckApplyInfoUI()
{
	pCheckApplyInfo =new CheckApplyInfo();
}
void CheckApplyInfoUI::checkApplyInfo()
{
	...
	ApplyInfoList* applyInfoList = pCheckApplyInfo->getApplyInfoList(); //지원정보 리스트 반환
	fprintf(out_fp, "> %s", applyInfoList->getApplyInfo() );
}
//CheckApplyInfoUI
CancelApplyInfoUI::CancelApplyInfoUI()
{
	pCancelApplyInfo = new CancelApplyInfo();
}
void CancelApplyInfoUI::cancelApply()
{
	//사업자번호로 applylist에서 찾고 
	//applylist에서 삭제
	//pCancelApplyInfo->cancelApply();//취소한 업무의 문자열 반환
	char SSN[MAX_STRING];
	fscanf(in_fp, "%s", SSN);
	...
	// 4 4 [사업자 번호]
	//	> [회사이름][사업자번호][업무]
	fprintf(out_fp, "> %s\n", pCancelApplyInfo->cancelApply(SSN));
}
//CancelApplyInfoUI

ApplyStatisticsUI::ApplyStatisticsUI()
{
	pApplyStatistics - new ApplyStatistics();
}
void ApplyStatisticsUI::showStatistics()
{
	...
	//fprintf(out_fp, "%s\n", pApplyStatistics->getStatistics());
}